            ...
            match message {
                InjectorPoolProtocol::RequestDeviceId { service_id} => {
                    match self.device_id_api.get_opt(&service_id).await {
                        Ok(None) => {
                            info!("Device ID for {} is not available yet. Retrying.", service_id);
                        }
                        Ok(device_id) => {
                            ...
                            for replicaset in replicasets {
                                if let Some(replicaset_owners) = &replicaset.metadata.owner_references.as_ref() {
                                    ...
                                    let replicaset_name = format!("{}-{}", replicaset.metadata.namespace.unwrap(), replicaset_owner.name);
                                    ...
                                    if replicaset_name == deployment_name {
                                        info!("Found owner for replicaset {}: deployment {}", replicaset_name, replicaset_owner.name);
                                        ...
                                        for pod in pods {
                                            if let Some(pod_owners) = &pod.metadata.owner_references.as_ref() {
                                                ...
                                                if pod_owner.name == replicaset_name {
                                                    info!("Found owner for pod {}: replicaset {}", pod_name,  pod_owner.name);
                                                    ...
                                                    for container in &pod.spec.as_ref().unwrap().containers {
                                                        if container.name == "sdp-service" {
                                                            for env in container.env.as_ref().unwrap() {
                                                                // If found, remove the uuid from availability
                                                                if env.name == "CLIENT_DEVICE_ID" {
                                                                    let unavailable_uuid = env.value.as_ref().unwrap();
                                                                    let index = available_uuids.iter()
                                                                        .position(|x| x == unavailable_uuid)
                                                                        .unwrap();
                            ...
                            }).await.expect("Error when sending FoundDeviceId message to Injector")
                        }
                        Err(err) => {
