...
use sdp_common::annotations::{SDP_ANNOTATION_CLIENT_CONFIG, SDP_ANNOTATION_CLIENT_DEVICE_ID, SDP_ANNOTATION_CLIENT_SECRETS, SDP_ANNOTATION_DNS_SEARCHES, SDP_INJECTOR_ANNOTATION_CLIENT_VERSION, SDP_INJECTOR_ANNOTATION_DISABLE_INIT_CONTAINERS, SDP_INJECTOR_ANNOTATION_ENABLED, SDP_INJECTOR_ANNOTATION_STRATEGY};
...
async fn patch_deployment(
    ...
    let injection_enabled = deployment
        ...
        .to_string();
    patches.push(Add(AddOperation {
        path: format!("/metadata/annotations/{}", patch_annotation!(SDP_INJECTOR_ANNOTATION_STRATEGY)),
        value: serde_json::to_value(injection_strategy.to_string())
            .map_err(|e| SDPServiceError::from(e))
            .map_err(SDPPatchError::from_admission_response(Box::clone(
                &admission_response,
    ...
    patches.push(Add(AddOperation {
        path: format!("/metadata/annotations/{}", patch_annotation!(SDP_INJECTOR_ANNOTATION_ENABLED)),
...
POD is missing needed volumes: pod-info, run-sdp-dnsmasq, run-sdp-driver, tun-device"#.to_string()),
    ...
    fn test_service_environment_from_pod() {
        ...
        if let Ok(Some(env)) = env {
            ...
            assert_eq!(
                env.client_secret_controller_url_key,
                "ns0_srv0-password".to_string()
            );
            assert_eq!(env.client_secret_pwd_key, "ns0_srv0-password".to_string());
            assert_eq!(env.client_secret_user_key, "ns0_srv0-user".to_string());
            assert_eq!(env.n_containers, "0".to_string());
            assert_eq!(env.k8s_dns_service_ip, None);
        };
        ...
        if let Ok(Some(env)) = env {
            ...
            assert_eq!(
                env.client_secret_controller_url_key,
                "ns1_srv1-password".to_string()
            );
            assert_eq!(env.client_secret_pwd_key, "ns1_srv1-password".to_string());
            assert_eq!(env.client_secret_user_key, "ns1_srv1-user".to_string());
            assert_eq!(env.n_containers, "0".to_string());
            assert_eq!(env.k8s_dns_service_ip, None);
        };
